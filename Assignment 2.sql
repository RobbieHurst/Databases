...
DECLARE
    ...
    CURSOR MedicineCursor IS SELECT FIRST_NAME || ',' ||SURNAME AS NAME, BILLING_TABLE.BILL_ID, MEDICINE_TABLE.MED_NAME, MED_PRICE
                            FROM TREATMENT_TABLE,BILLING_TABLE,CUSTOMER_TABLE,MEDICINE_TABLE
...
DECLARE
    ...
    CURSOR DiscountCursor IS SELECT CUSTOMER_TABLE.CUSTOMER_ID,DISEASE_TABLE.DISEASE_NAME, ANIMAL_TABLE.ANIMAL_TYPE, MEDICINE_TABLE.MED_NAME,MEDICINE_TABLE.MED_PRICE
                              FROM TREATMENT_TABLE,BILLING_TABLE,CUSTOMER_TABLE,MEDICINE_TABLE,DISEASE_TABLE,ANIMAL_TABLE
                                WHERE BILLING_TABLE.BILL_ID = TREATMENT_TABLE.BILL_ID
                                      AND CUSTOMER_TABLE.CUSTOMER_ID = BILLING_TABLE.CUSTOMER_ID
                                      AND MEDICINE_TABLE.MED_ID = TREATMENT_TABLE.MED_ID
    ...
    BEGIN
      OPEN DiscountCursor;
      LOOP
        FETCH DiscountCursor INTO DISPLAY_CUSTOMER, DISPLAY_DISEASE, DISPLAY_TYPE, DISPLAY_MEDICINE, DISPLAY_PRICE;
...
END;
--Question 3
SELECT TREATMENT_TABLE.VET_ID, SPECIALITY_TABLE.SPECIALITY, ANIMAL_TABLE.ANIMAL_TYPE, DISEASE_TABLE.DISEASE_NAME, MEDICINE_TABLE.MED_NAME,BILLING_TABLE.STAFF_ID, CUSTOMER_TABLE.CUSTOMER_ID, BILLING_TABLE.BILL_ID
  FROM TREATMENT_TABLE,BILLING_TABLE,CUSTOMER_TABLE,MEDICINE_TABLE,DISEASE_TABLE,ANIMAL_TABLE,VETERINARIAN_TABLE,SPECIALITY_TABLE
    WHERE BILLING_TABLE.BILL_ID = TREATMENT_TABLE.BILL_ID
          AND CUSTOMER_TABLE.CUSTOMER_ID = BILLING_TABLE.CUSTOMER_ID
          AND MEDICINE_TABLE.MED_ID = TREATMENT_TABLE.MED_ID
          AND DISEASE_TABLE.DISEASE_ID = TREATMENT_TABLE.DISEASE_ID
